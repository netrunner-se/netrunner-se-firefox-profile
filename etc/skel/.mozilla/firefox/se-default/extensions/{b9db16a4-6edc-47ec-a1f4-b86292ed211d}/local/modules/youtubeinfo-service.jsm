...
function YTInfo() {
	try {
		...
		this.formats={
				...
				133: { type: "133", format: "MP4", video: "320x240", audio: "stereo", name: "HQ133" },
				134: { type: "134", format: "MP4", video: "480x360", audio: "stereo", name: "HQ134" },
				135: { type: "135", format: "MP4", video: "640x480", audio: "stereo", name: "HQ135" },
				...
				160: { type: "160", format: "MP4", video: "192x144", audio: "stereo", name: "HQ160" },
		}
					...
					for(var i=0;i<adpEntries.length;i++) {
						...
						if(entry.itag && entry.url) {
							...
							if(!(format in selFormats)) {
								//dump("skip "+format+": "+JSON.stringify(entry,null,2)+"\n");
								continue;
							}
							var url;
							if(entry.s) {
								...
								try {
									...
									dump("Not found yt perm for "+"yt-sig-perm-"+entry.s.length+"\n");
								}
								var sigParts=[];
								...
								/*
								if(entry.s.length==86) {
									sig=entry.s.substring(2,entry.s.length-3);
									var c61=sig.substring(61,62);
									var c80=sig.substring(80,81);
									sig=sig.substring(0,61)+c80+sig.substring(62,80)+c61;
								} else if(entry.s.length==83) {
									//if(entry.s.substring(0,2)==entry.s.substring(2,4))
									//	sig=entry.s.substring(0,entry.s.length-2);
									//else
									//	sig=entry.s;
									var c0=entry.s.substring(0,1);
									var c2=entry.s.substring(2,3);
									var c6=entry.s.substring(6,7);
									var c24=entry.s.substring(24,25);
									var c33=entry.s.substring(33,34);
									var c53=entry.s.substring(53,54);
									sig= c6 +
										entry.s.substring(3,6) +
										c33 +
										entry.s.substring(7,24) +
										c0 +
										entry.s.substring(25,33) +
										c2 +
										entry.s.substring(34,53) +
										c24 +
										entry.s.substring(54,83);
									if(this.pref.getBoolPref("yt-log"))
										dump(">>> "+sig+"\n");
								} else
									continue;
